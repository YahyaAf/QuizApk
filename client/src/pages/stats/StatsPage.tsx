...
function CustomTooltip({ active, payload, label }: any) {
  if (active && payload?.length) return (
    <div className="card" style={{ padding:'8px 14px', fontSize:12, border:'1px solid #DDE8F0' }}>
      <div style={{ fontWeight:700, color:'#053F5C' }}>{label}</div>
      <div style={{ color:'#429EBD' }}>Score : <strong style={{ color:'#053F5C' }}>{Math.round(payload[0].value)}%</strong></div>
    </div>
  );
  return null;
}
...
export default function StatsPage() {
  ...
  const scoreHistory = results
    .map(r => ({
      month: new Date(r.createdAt).toLocaleString('fr-FR', { month: 'short', day: 'numeric' }),
  ...
  // Derive subjectScores from results
  const subjectMap: Record<string, number> = {};
  results.forEach(r => {
    const s = r.courseName || r.examTitle;
    if (!subjectMap[s] || r.percentage > subjectMap[s]) {
      subjectMap[s] = r.percentage || 0;
    }
  });
  const subjectScores = Object.entries(subjectMap).map(([subject, score]) => ({ subject, score }));
    ...
    <div className="animate-fade-in" style={{ display:'flex', flexDirection:'column', gap:24, width:'100%' }}>
      ...
      {user?.role === 'ROLE_STUDENT' && (
        <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:20 }}>
          ...
          {/* Bar Chart */}
          <div className="card" style={{ padding:24 }}>
            <h2 style={{ fontWeight:700, color:'#053F5C', fontSize:15, marginBottom:20 }}>Par matière</h2>
            <ResponsiveContainer width="100%" height={230}>
              <BarChart data={subjectScores} layout="vertical" margin={{top:0,right:10,left:0,bottom:0}}>
                <CartesianGrid strokeDasharray="3 3" stroke="#EBF2F8" horizontal={false} />
                <XAxis type="number" domain={[0,100]} tick={{fontSize:11, fill:'#6B9AB8'}} axisLine={false} tickLine={false} />
                <YAxis dataKey="subject" type="category" tick={{fontSize:11, fill:'#6B9AB8', fontWeight:600}} axisLine={false} tickLine={false} width={68} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="score" radius={[0,7,7,0]}>
                  {subjectScores.map((e,i) => (
                    <Cell key={i} fill={e.score>=85?'#053F5C':e.score>=60?'#429EBD':e.score>0?'#F7AD19':'#EBF2F8'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
