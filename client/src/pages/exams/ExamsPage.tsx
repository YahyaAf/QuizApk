...
const statusLabel: Record<string, { label: string; bg: string; color: string }> = {
  upcoming:    { label: 'À venir',  bg:'#EBF5FB', color:'#053F5C' },
  active:      { label: 'En cours', bg:'#FEF9EC', color:'#8C5D07' },
  completed:   { label: 'Terminé',  bg:'#DDF1F9', color:'#1A6882' },
  missed:      { label: 'Manqué',   bg:'#FFF1F2', color:'#BE123C' },
  devalidated: { label: 'Dévalidé', bg:'#FEE2E2', color:'#991B1B' },
  disabled:    { label: 'Désactivé', bg:'#F3F4F6', color:'#4B5563' },
};
  ...
  return (
    <div className="card-interactive" style={{ padding:'16px 22px', display:'flex', alignItems:'center', gap:18, justifyContent:'space-between' }}>
      ...
      {exam.status === 'completed' && exam.score !== undefined ? (
        ...
        >
          <PlayCircle size={15} />
          {exam.status === 'devalidated' ? 'Dévalidé' : (exam.status === 'disabled' ? 'Désactivé' : (exam.status === 'active' ? 'Démarrer' : 'Indisponible'))}
        </button>
      )}
    </div>
  );
...
export default function ExamsPage() {
  ...
  const filtered = exams.filter((e) => {
    const ms = filter === 'all' 
      || (filter === 'upcoming' && ['active', 'disabled', 'devalidated'].includes(e.status))
      || e.status === filter;
        ...
        <div style={{ display:'flex', gap:4, background:'#F0F4F8', borderRadius:10, padding:4 }}>
          {tabs.map((t) => {
            const cnt = t.key==='all' 
              ? exams.length 
              : t.key==='upcoming'
                ? exams.filter(e=>['active', 'disabled', 'devalidated'].includes(e.status)).length
